...
import { PaymentOutboxEvent } from "./database.js";

const OUTBOX_SERVICE_NAME = "payment-service";
...
export async function enqueueOutboxEvent({
  ...
  try {
    const outboxEvent = new PaymentOutboxEvent({
    ...
    if (error?.code === 11000) {
      const query = PaymentOutboxEvent.findOne({
        service: OUTBOX_SERVICE_NAME,
        topic,
        event_key: eventKey,
      });
...
export function triggerOutboxFlush() {
  void flushOutbox().catch((error) => {
    console.error("Payment outbox flush trigger error:", error);
  });
}
...
export function startOutboxProcessor(intervalMs = 5_000) {
  ...
  const run = async () => {
    try {
      ...
      console.error("Payment outbox flush error:", error);
    }
  };
...
export async function startConsumer() {
  console.log("Payment service no longer consumes booking-created to auto-complete demo payments.");
}
